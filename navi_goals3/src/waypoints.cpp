...
//Function received command from web browser
...
bool buildWaypointsFromFile(std::vector<geometry_msgs::PointStamped> &waypoints) // pass array of PointStamped messages by reference
{
  ...
  std::string pkg_path = ros::package::getPath( "navi_goals3" ); // change package name accordingly
...
void run(std::string ref_frame, move_base_msgs::MoveBaseGoal &goal, const std::vector<geometry_msgs::PointStamped> &waypoints, int index,geometry_msgs::PoseStamped &msg)
{
  ...
  while ( !action_client.waitForServer( ros::Duration(5.0) ) )
  {
    ROS_INFO("Waiting for the move_base action server to come up");
	  ...
	  //Pose (point)
	  msg.pose.position.x= waypoints[index].point.x;
	  msg.pose.position.y= waypoints[index].point.y;
